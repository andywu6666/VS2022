...
public:
      ...
      : myData()
   {
      ...
      if( count > 0 )
      {
          //nodePtr newNode = new node; //move from
   ...
   void resize( size_type newSize )
   {
      if( myData.mySize != newSize )
      {
         ...
         {
            // erase myData.mySize - newSize elements

             size_type shift = myData.mySize - newSize;

             for (size_type j = 0; j < shift; j++) //modify
             {
                 nodePtr nodeToDelete = myData.myHead->prev; //modify
                 nodePtr prevNode = nodeToDelete->prev; //modify
                 //nodePtr nextNode = new node; //delete
                // nodeToDelete = prevNode->prev; //delete
                 prevNode->next = myData.myHead; //modify
                 myData.myHead->prev = prevNode; //modify
                 /*prevNode = nodeToDelete->prev; //delete
                 nextNode = nodeToDelete->next;*/ //delete
