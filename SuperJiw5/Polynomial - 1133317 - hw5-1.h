...
public:
   ...
   {
       vector< T1 > merged;
       auto it1 = polynomial.begin();
       auto it2 = op2.polynomial.begin();
       ...
       while (it1 != polynomial.end() && it2 != op2.polynomial.end())
       {
           ...
           else // equal exponents
           {
               ...
               if (sumCoef != 0)
                   merged.insert(merged.end(), T1{ sumCoef, it1->expon });
               ++it1;
               ++it2;
           }
       }
   ...
   {
       Polynomial product;
       for (auto it1 = polynomial.begin(); it1 != polynomial.end(); ++it1)
           for (auto it2 = op2.polynomial.begin(); it2 != op2.polynomial.end(); ++it2)
           {
               ...
               bool inserted = false;
               for (auto it = product.polynomial.begin(); it != product.polynomial.end(); ++it)
               {
                   if (it->expon == expon)
                   {
                       it->coef += coef;
                   ...
                   {
                       product.polynomial.insert(it, T1{ coef, expon });
                       inserted = true;
                       break;
                   }
               }
